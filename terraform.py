...
@calculate_mi_vars
def triton_host(resource, module_name):
    name = resource['primary']['attributes']['name']
    raw_attrs = resource['primary']['attributes']
    ...
    attrs = {
        'administrator_pw': raw_attrs['administrator_pw'],
        'created': raw_attrs['created'],
        'dataset': raw_attrs['dataset'],
        'disk': raw_attrs['disk'],
        'firewall_enabled': raw_attrs['firewall_enabled'],
        'id': raw_attrs['id'],
        'image': raw_attrs['image'],
        'ips.#': raw_attrs['ips.#'],
        'ips.0': raw_attrs['ips.0'],
        'ips.1': raw_attrs['ips.1'],
        'memory': raw_attrs['memory'],
        'name': raw_attrs['name'],
        'networks.#': raw_attrs['networks.#'],
        'networks.0': raw_attrs['networks.0'],
        'networks.1': raw_attrs['networks.1'],
        'package': raw_attrs['package'],
        'primaryip': raw_attrs['primaryip'],
        'root_authorized_keys': raw_attrs['root_authorized_keys'],
        'state': raw_attrs['state'],
        'tags.#': raw_attrs['tags.#'],
        'tags.role': raw_attrs['tags.role'],
        'type': raw_attrs['type'],
        'updated': raw_attrs['updated'],
        'user_data': raw_attrs['user_data'],
        'user_script': raw_attrs['user_script'],
        # ansible
        'ansible_ssh_port': 22,
        'ansible_ssh_host': raw_attrs['ips.0'],
        # generic
        'public_ipv4': raw_attrs['ips.0'],
        'private_ipv4': raw_attrs['ips.1'],
        'provider': 'triton',
    }
    # attrs specific to microservices-infrastructure
    attrs.update({
        'consul_dc': attrs['user_data'].lower().replace(" ", "-"),
        'role': attrs['tags.role']
    })

    # groups specific to microservices-infrastructure
    groups.extend(['triton_image=' + attrs['image'],
                   'triton_location=' + attrs['tags.role'].lower().replace(" ", "-")])

    # groups specific to microservices-infrastructure
    groups.append('role=' + attrs['tags.role'])
    groups.append('dc=' + attrs['consul_dc'])
